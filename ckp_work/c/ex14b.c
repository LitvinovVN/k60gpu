...
//
  ...
  if (mp == 0) fprintf(stderr,"Time loop start...\n");
...
//
  do {
    ...
    // step 1:
    if (lp>0) fprintf(stderr,"[%d] ntv=%d step 1\n",mp,ntv);
    ...
    // step 2:
    if (lp>0) fprintf(stderr,"[%d] ntv=%d step 2\n",mp,ntv);

    //...

    if (lp>0) fprintf(stderr,"[%d] check output\n",mp);
    ...
    if (ntv % ntp == 0) {
      ...
      for (m=0; m<nc12; m++) {
        s0 = dabs(yy0[m]);
        if (s0>1e-15) s0 = (yy1[m]/yy0[m]-1.0); else s0 = 0.0;
        gt = dmax(gt,dabs(s0));
      }
