...
int main(int argc, char *argv[])
{
  ...
  double s0, s1, s2, s3, s4, s1m, s1p, s2m, s2p;
  ...
  eps = pow(s0,3);
  eps = dmax(eps,1e-14);
  if (argc>6) sscanf(argv[6],"%le",&eps);
  ...
  tau0 = 0.25;
  if (argc>7) sscanf(argv[7],"%le",&tau0);
  tau = tau0;

  mc = (int)(0.5*log(1.0*n12p)/log(2.0));
  if (argc>8) sscanf(argv[8],"%d",&mc);
...
// Iterative loop:
      ...
      if ((i2>0) && (i2<n2)) {
        for (k1=0; k1<nc1; k1++) {
          ...
          if ((i1>0) && (i1<n1)) {
            ...
            s2 = g12 * (s2m + s2p);
            s3 = h12 * ff[m];
            s4 = s1 + s2 + s3 - s0; // residual
            rka = dmax(rka,dabs(s4));
            yy1[m] = s4;
          }
        }
      }
  ...
  for (k2=0; k2<nc2; k2++) {
    ...
    for (k1=0; k1<nc1; k1++) {
      ...
      s1 = dabs(yy1[m]-s0);
      dka = dmax(dka,s1);
      if (lp>1) fprintf(Fo,"i1=%8d i2=%8d y=%le u=%le d=%le\n",i1,i2,yy1[m],s0,s1);
    }
  }
