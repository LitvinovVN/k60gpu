...
int main(int argc, char *argv[])
{
  ...
  if (mp==0) {
    fprintf(stderr,"Usage: mpirun -np <process number> ex11a.px [<grid cells number>]\n");
  ...
  fprintf(Fo,"np=%d mp=%d nx=%d --> i1=%d i2=%d nc=%d ncm=%d ncp=%d ncx=%d\n",
    np, mp, nx, i1, i2, nc, ncm, ncp, ncx);
